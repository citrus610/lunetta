...
fn bumpiness(heights: &[u32; 10], well_x: usize) -> i32 {
    ...
    for i in 1..10 {
        ...
        bumpiness += (heights[left] - heights[i]) * (heights[left] - heights[i]);
        left = i;
    }
...
fn tslot(board: &Board, heights: &[u32; 10]) -> Option<Move> {
    for x in 0..8 {
        ...
        if heights[x + 1] >= heights[x] && heights[x + 1] + 1 < heights[x + 2] {
        ...
        if heights[x + 1] >= heights[x + 2] && heights[x + 1] + 1 < heights[x] {
