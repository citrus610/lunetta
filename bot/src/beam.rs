...
fn expand(node: &Node, queue: &[Piece], mut callback: impl FnMut(Node, Move)) -> usize {
    ...
    let hold = node.state.hold.unwrap_or_else(|| queue[node.state.next + 1]);
...
pub fn beam_search(
    ...
    let mut parents = Vec::with_capacity(settings.width);
    ...
    // Sort candidates
    result.candidates.sort_by_key(|c| Reverse(c.reward));
